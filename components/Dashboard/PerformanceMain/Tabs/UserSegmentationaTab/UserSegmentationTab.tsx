...
const UserSegmentationTab = ({ data }: { data: any }) => {
   const cardsData = useMemo<ICard[]>(() => [
    { label: "Geolocation", type: CardType.USER_SEGMENTATION, data: data?.geolocation || []},
    { label: "FCL/LCL", type: CardType.USER_SEGMENTATION, data: data?.placementOfGoods || []},
  ], [data]);
  ...
  return (
    <>
      <PerformanceCards data={cardsData} onChangeActiveCard={handleActiveCardChange} activeCard={activeCard}/>
