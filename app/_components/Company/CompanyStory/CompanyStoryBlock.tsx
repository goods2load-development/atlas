  ...
  return (
    <div className='w-3/12 flex flex-col gap-[24px]'>
      ...
      <div className='flex flex-col text-black'>
        <h2 className='font-normal text-[26px]/[22px] italic pb-[16px]'>{item.time}</h2>
        {!item.more ?
          <p className='font-normal text-[16px]/[22.4px]'>{item.info}</p>
          :
          <p className={`font-normal text-[16px]/[22.4px] ${!showMore && 'max-h-[156px] text-story w-full'}`}>{item.info}</p>
        }
        {item.more && <p className='text-[#FF6720] text-[15px]/[18px] font-medium underline underline-offset-1 decoration-[1.5px] pt-[4px] hover:cursor-pointer' onClick={()=>{setShowMore(!showMore)}}>{!showMore ? "Learn more" : "Hide"}</p>}
      </div>
    </div>
  );
};
